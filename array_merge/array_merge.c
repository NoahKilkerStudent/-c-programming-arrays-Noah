...
#include <stdlib.h>
//declaim
int arraySizeCount(int, int* );
int* arrayCombination(int, int*, int**);
int uniqueNumsFind(int, int* );
int* combineResults(int, int* );
...
int* array_merge(int num_arrays, int* sizes, int** values) {
  ...
  int* arrayFinal = combineResults(arraySizeResult, arrayCombinationResult);
...
int uniqueNumsFind(int arraySize, int* values) {
    if (arraySize == 0){
        return 0;
    }
...
int* combineResults(int arraySize, int* values) {
    int specNums = uniqueNumsFind(arraySize, values);
